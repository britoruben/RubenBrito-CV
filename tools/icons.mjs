 ...
 * Inline SVG icons.
 *
 * These replace the Font Awesome CDN stylesheet. Loading ~15 glyphs from
 * cdnjs cost a render-blocking stylesheet plus font files, and any network
 * that blocks the CDN (common on corporate banking networks) stripped every
 * icon from the page - including the theme toggle, which then rendered as an
 * empty circle with no visible label.
 *
 * Outline icons are stroke-based on a 24x24 grid and inherit currentColor.
 * Brand marks (GitHub, LinkedIn) are filled paths, as their trademarks require.
 */
...
/**
 * Returns an inline <svg>. Decorative by default (aria-hidden), since every
 * icon here sits next to its own text label.
 */
export function icon(name, extraClass = '') {
  const cls = extraClass ? `icon ${extraClass}` : 'icon';
  const common = `class="${cls}" viewBox="0 0 24 24" width="16" height="16" aria-hidden="true" focusable="false"`;
  if (OUTLINE[name]) {
    return `<svg ${common} fill="none" stroke="currentColor" stroke-width="1.8" `
      + `stroke-linecap="round" stroke-linejoin="round">${OUTLINE[name]}</svg>`;
  }
  if (FILLED[name]) {
    return `<svg ${common} fill="currentColor">${FILLED[name]}</svg>`;
  }
